...
class QwenThoughtModel( ThoughtModel ):
	...
	def __init__( self, config: QwenThoughtModelConfig ):
		...
		if config.host_thought_token_embeddings:
			self.start_token_embedding = t.nn.Parameter( self._lm_model.get_input_embeddings().weight.data[ self.start_thought_token ] )
			self.end_token_embedding = t.nn.Parameter( self._lm_model.get_input_embeddings().weight.data[ self.end_thought_token ] )
		#
